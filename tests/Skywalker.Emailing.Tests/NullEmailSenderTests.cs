...
namespace Skywalker.Emailing.Tests;

public class NullEmailSenderTests
{
    private static IOptions<EmailSenderOptions> CreateOptions(
        string fromAddress = "noreply@test.com", string fromName = "Test")
    {
        var opts = Substitute.For<IOptions<EmailSenderOptions>>();
        opts.Value.Returns(new EmailSenderOptions
        {
    ...
    public async Task QueueAsync_DelegatesToSendAsync()
    {
        ...
        // QueueAsync should not throw 鈥?it delegates to SendAsync
