...
using Skywalker.Aspects.DynamicProxy.Contributors;
using Skywalker.Aspects.DynamicProxy.Generators.Emitters;
using Skywalker.Aspects.DynamicProxy.Generators.Emitters.CodeBuilders;
    ...
    public abstract class BaseProxyGenerator
    {
        ...
        protected void EnsureOptionsOverrideEqualsAndGetHashCode(ProxyGenerationOptions options)
        {
            if (Logger.IsEnabled(LogLevel.Warning))
            {
                ...
                {
                    Logger.LogWarning("The IProxyGenerationHook type {0} does not override both Equals and GetHashCode. " +
        ...
        protected Type ObtainProxyType(CacheKey cacheKey, Func<string, INamingScope, Type> factory)
        {
            ...
            using (var locker = Scope.Lock.ForReading())
            {
                ...
                if (cacheType != null)
                {
                    Logger.LogDebug("Found cached proxy type {0} for target type {1}.", cacheType.FullName, targetType.FullName);
                    return cacheType;
                }
            }
