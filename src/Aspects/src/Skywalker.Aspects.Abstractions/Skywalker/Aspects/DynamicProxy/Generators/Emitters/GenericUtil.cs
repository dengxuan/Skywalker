...
namespace Skywalker.Aspects.DynamicProxy.Generators.Emitters
{
    ...
    internal class GenericUtil
    {
        ...
        public static Type ExtractCorrectType(Type paramType, Dictionary<string, GenericTypeParameterBuilder> name2GenericType)
        {
            if (paramType.GetTypeInfo().IsArray)
            {
                ...
                {
                    if (name2GenericType.TryGetValue(underlyingType.Name, out GenericTypeParameterBuilder genericType) == false)
                    {
                        return paramType;
                    }
            ...
            {
                if (name2GenericType.TryGetValue(paramType.Name, out GenericTypeParameterBuilder value))
                {
                    return value;
                }
            }
