    ...
    public abstract class MembersCollector
    {
        ...
        protected bool AcceptMethod(MethodInfo method, bool onlyVirtuals, IProxyGenerationHook hook)
        {
            ...
            if (onlyVirtuals && !isOverridable)
            {
                ...
                {
                    Logger.LogDebug("Excluded non-overridable method {0} on {1} because it cannot be intercepted.", method.Name,
            ...
            if (method.IsFinal)
            {
                Logger.LogDebug("Excluded sealed method {0} on {1} because it cannot be intercepted.", method.Name,
                                   method.DeclaringType.FullName);
                return false;
            }
