...
namespace Skywalker.Lightning.Terminal.Proxies
{
    public class LightningProxyGenerator : ILightningProxyGenerator, IDisposable
    {
        ...
        public LightningProxyGenerator(ILightningServiceNameGenerator LightningServiceNameGenerator, ILoggerFactory loggerFactory)
        {
            _logger = loggerFactory.CreateLogger<LightningProxyGenerator>();
        ...
        public IEnumerable<Type> GenerateProxy(IEnumerable<Type> interfaceTypes)
        {
            ...
            var stream = CompilationUtilitys.CompileTerminalProxy(trees, assemblies.Select(x => MetadataReference.CreateFromFile(x.Location))
                .Concat(new[]
                {
        ...
        {
            var className = interfaceType.Name.StartsWith("I") ? interfaceType.Name[1..] : interfaceType.Name;
            className += "TerminalProxy";
                    ...
                    SyntaxFactory.SingletonList<MemberDeclarationSyntax>(
                        SyntaxFactory.NamespaceDeclaration(
                            SyntaxFactory.QualifiedName(
                                SyntaxFactory.QualifiedName(
                                    SyntaxFactory.QualifiedName(
                                        SyntaxFactory.QualifiedName(SyntaxFactory.IdentifierName("Skywalker"), SyntaxFactory.IdentifierName("Lightning")),
                                    SyntaxFactory.IdentifierName("Terminal")),
                                SyntaxFactory.IdentifierName("Proxies")),
                            SyntaxFactory.IdentifierName("DynamicProxies")))
                  .WithMembers(
                      SyntaxFactory.SingletonList<MemberDeclarationSyntax>(
                              ...
                              .WithBaseList(
                                  SyntaxFactory.BaseList(
                                      SyntaxFactory.SeparatedList<BaseTypeSyntax>(
                                          ...
                                          {
                                            SyntaxFactory.SimpleBaseType(SyntaxFactory.IdentifierName("LightningProxy")),
        ...
        private static SyntaxList<UsingDirectiveSyntax> GetUsings()
        {
            return SyntaxFactory.List(
                new[]
                {
                    ...
                    SyntaxFactory.UsingDirective(GetQualifiedNameSyntax(typeof(ILightningInvoker).Namespace)),
                });
        }
            ...
            return SyntaxFactory.ConstructorDeclaration(SyntaxFactory.Identifier(className))
                ...
                .WithParameterList(
                   SyntaxFactory.ParameterList(
                       SyntaxFactory.SeparatedList<ParameterSyntax>(
                            new SyntaxNodeOrToken[]
                            {
                                SyntaxFactory.Parameter(SyntaxFactory.Identifier("LightningInvoker")).WithType(SyntaxFactory.IdentifierName("ILightningInvoker")),
                                SyntaxFactory.Token(SyntaxKind.CommaToken)
                            }
                           )
                       )
                ...
                SyntaxFactory.ArgumentList(
                    SyntaxFactory.SeparatedList<ArgumentSyntax>(
                        new SyntaxNodeOrToken[]
                        {
                            SyntaxFactory.Argument(SyntaxFactory.IdentifierName("LightningInvoker")),
                            SyntaxFactory.Token(SyntaxKind.CommaToken)
                        }
                        )
                    )
        ...
        {
            var serviceName = _LightningServiceNameGenerator.GetLightningServiceName(method, method.GetParameters());
            ...
            if (method.ReturnType.Namespace != typeof(Task).Namespace)
            {
                ...
                {
                    expressionSyntax = SyntaxFactory.GenericName(SyntaxFactory.Identifier("Invoke")).WithTypeArgumentList(SyntaxFactory.TypeArgumentList(SyntaxFactory.SingletonSeparatedList(returnDeclaration)));
                }
                expressionSyntax = SyntaxFactory.InvocationExpression(expressionSyntax).WithArgumentList(
                                   ...
                                   {
                                        SyntaxFactory.Argument(SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal(serviceName))),
                                        SyntaxFactory.Token(SyntaxKind.CommaToken),
                                        SyntaxFactory.Argument(SyntaxFactory.ObjectCreationExpression(SyntaxFactory.GenericName(SyntaxFactory.Identifier("Dictionary"))
                                            ...
                                            .WithInitializer(SyntaxFactory.InitializerExpression(SyntaxKind.CollectionInitializerExpression,SyntaxFactory.SeparatedList<ExpressionSyntax>(parameterList))))
                                    })));
            ...
            else
            {
                ...
                {
                    expressionSyntax = SyntaxFactory.GenericName(SyntaxFactory.Identifier("InvokeAsync")).WithTypeArgumentList(((GenericNameSyntax)returnDeclaration).TypeArgumentList);
                }
