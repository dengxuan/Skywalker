using Skywalker.Spider.Pipeline.DataResolver.Abstractions;
...
namespace Skywalker.Spider.Pipeline.DataResolver;
...
public sealed class Parser
{
    ...
    public static T Parse<TGenerator, T>(string selectors, TGenerator generator, Func<TGenerator, T> resultor)
        where TGenerator : ISelectorGenerator
    {
        string validSelectors = selectors.NotNullOrEmpty(nameof(selectors));
        IEnumerable<Token> token = Tokener.Tokenize(validSelectors);
        return Parse(token, generator, resultor);
    }
    ...
    public static T Parse<TGenerator, T>(IEnumerable<Token> tokens, TGenerator generator, Func<TGenerator, T> resultor)
        where TGenerator : ISelectorGenerator
    {
        tokens.NotNull(nameof(tokens));
        resultor.NotNull(nameof(resultor));

        new Parser(new Reader<Token>(tokens), generator).Parse();
    ...
    {
        if (!CustomSelectors.TryGetValue(name, out DelegateInfo? deleg))
        {
