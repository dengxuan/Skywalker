...
namespace Skywalker.Extensions.Filters;
...
public sealed class BloomFilter
{
    ...
    private static readonly KeyedHashAlgorithm s_keyedHashAlgorithm = new HMACSHA1();
    ...
    {
        return CreateHash(val, s_charset);
    }
    ...
    public static int[] CreateHashes(byte[] data, int hashes)
    {
        ...
        {
            byte[] digest;
            lock (s_keyedHashAlgorithm)
            {
                s_keyedHashAlgorithm.Key = salt;
                salt[0]++;
                digest = s_keyedHashAlgorithm.ComputeHash(data);
            }
    ...
    {
        Add(s_charset.GetBytes(element.ToString()!));
    }
    ...
    {
        return Contains(s_charset.GetBytes(element.ToString()!));
    }
