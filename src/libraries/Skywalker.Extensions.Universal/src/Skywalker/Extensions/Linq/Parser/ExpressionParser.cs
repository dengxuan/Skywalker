...
{
    private static readonly string s_methodOrderBy = nameof(Queryable.OrderBy);
    private static readonly string s_methodOrderByDescending = nameof(Queryable.OrderByDescending);
    private static readonly string s_methodThenBy = nameof(Queryable.ThenBy);
    private static readonly string s_methodThenByDescending = nameof(Queryable.ThenByDescending);
    ...
    internal IList<DynamicOrdering> ParseOrdering(bool forceThenBy = false)
    {
        ...
        while (true)
        {
            ...
            {
                methodName = ascending ? s_methodThenBy : s_methodThenByDescending;
            }
            else
            {
                methodName = ascending ? s_methodOrderBy : s_methodOrderByDescending;
            }
    ...
    Expression ParseComparisonOperator()
    {
               ...
               _textParser.CurrentToken.Id == TokenId.LessThan || _textParser.CurrentToken.Id == TokenId.LessThanEqual)
        {
            ...
            if (isEquality && (!left.Type.GetTypeInfo().IsValueType && !right.Type.GetTypeInfo().IsValueType || left.Type == typeof(Guid) && right.Type == typeof(Guid)))
            {
                ...
                {
                    if (left.Type.IsAssignableFrom(right.Type) || ExpressionParser.HasImplicitConversion(right.Type, left.Type))
                    {
                        right = Expression.Convert(right, left.Type);
                    }
                    else if (right.Type.IsAssignableFrom(left.Type) || ExpressionParser.HasImplicitConversion(left.Type, right.Type))
                    {
            ...
            else
            {
                ...
                if (!typesAreSameAndImplementCorrectInterface)
                {
                    ...
                    {
                        if (ExpressionParser.HasImplicitConversion(left.Type, right.Type))
                        {
                            left = Expression.Convert(left, right.Type);
                        }
                        else if (ExpressionParser.HasImplicitConversion(right.Type, left.Type))
                        {
                            right = Expression.Convert(right, left.Type);
                        }
                    }
                    ...
                    {
                        if (ExpressionParser.HasImplicitConversion(right.Type, left.Type))
                        {
                            right = Expression.Convert(right, left.Type);
                        }
                        else if (ExpressionParser.HasImplicitConversion(left.Type, right.Type))
                        {
                            left = Expression.Convert(left, right.Type);
                        }
                    }
    ...
    private static bool HasImplicitConversion(Type baseType, Type targetType)
    {
        ...
        return targetType.GetMethods(BindingFlags.Public | BindingFlags.Static)
                         .Where(predicate => predicate.Name == "op_Implicit" && predicate.ReturnType == targetType)
                         .Any(predicate => predicate.GetParameters().FirstOrDefault()?.ParameterType == baseType);
    }
    ...
    {
        return _textParser.CurrentToken.Id switch
        {
    ...
    Expression ParseStringLiteral()
    {
        ...
        var result = StringParser.ParseString(_textParser.CurrentToken.Text);
...
#else
            if (float.TryParse(text.AsSpan(0, text.Length - 1), NumberStyles.Float, _parsingConfig.NumberParseCulture, out var f))
#endif
            {
...
#else
            if (decimal.TryParse(text.AsSpan(0, text.Length - 1), NumberStyles.Number, _parsingConfig.NumberParseCulture, out var d))
#endif
            {
...
#else
            if (double.TryParse(text.AsSpan(0, text.Length - 1), NumberStyles.Number, _parsingConfig.NumberParseCulture, out d))
#endif
            {
                return ConstantExpressionHelper.CreateLiteral(d, text);
            }
    ...
    Expression ParseFunctionNullPropagation()
    {
        ...
        if (_expressionHelper.ExpressionQualifiesForNullPropagation(args[0]))
        {
            ...
            if (_expressionHelper.TryGenerateAndAlsoNotNullExpression(args[0], hasDefaultParameter, out var generatedExpression))
            {
    ...
    Expression ParseLambdaInvocation(LambdaExpression lambda)
    {
        ...
        Expression? nullExpressionReference = null;
        if (_methodFinder.FindMethod(lambda.Type, nameof(Expression.Invoke), false, ref nullExpressionReference, ref args, out var _) != 1)
        {
    ...
    Expression ParseTypeAccess(Type type)
    {
        ...
        if (_textParser.CurrentToken.Id == TokenId.OpenParen || shorthand)
        {
            ...
            var constructorsWithOutPointerArguments = type.GetConstructors()
                                                          ...
                                                          .ToArray();
            switch (_methodFinder.FindBestMethod(constructorsWithOutPointerArguments, ref args, out var method))
            {
                case 0:
    ...
    {
        var exprType = sourceExpression.Type;
        if (exprType == type)
        {
        ...
        {
            var text = (string?)((ConstantExpression)sourceExpression).Value;
    ...
    {
        var isQueryable = TypeHelper.FindGenericType(typeof(IQueryable<>), type) != null;
        var isDictionary = TypeHelper.FindGenericType(typeof(IDictionary<,>), type) != null;
    ...
    private Type ResolveTypeFromArgumentExpression(string functionName, Expression argumentExpression)
    {
        return argumentExpression switch
        {
            ...
            {
                string typeName => ResolveTypeStringFromArgument(functionName, typeName),
                Type type => type,
                _ => throw ParseError(_textParser.CurrentToken.Pos, Res.FunctionRequiresOneNotNullArgOfType, functionName, "string or System.Type"),
            },
            _ => throw ParseError(_textParser.CurrentToken.Pos, Res.FunctionRequiresOneNotNullArgOfType, functionName, "ConstantExpression"),
        };
    }
    ...
    Expression ParseElementAccess(Expression expr)
    {
        ...
        return _methodFinder.FindIndexer(expr.Type, args, out var mb) switch
        {
            0 => throw ParseError(errorPos, Res.NoApplicableIndexer,
                                TypeHelper.GetTypeName(expr.Type)),
            1 => Expression.Call(expr, (MethodInfo)mb!, args),
            _ => throw ParseError(errorPos, Res.AmbiguousIndexerInvocation, TypeHelper.GetTypeName(expr.Type)),
        };
    }
    ...
    static string? GetOverloadedOperationName(TokenId tokenId)
    {
        ...
        {
            TokenId.DoubleEqual or TokenId.Equal => "op_Equality",
            TokenId.ExclamationEqual => "op_Inequality",
            _ => null,
        };
    }
