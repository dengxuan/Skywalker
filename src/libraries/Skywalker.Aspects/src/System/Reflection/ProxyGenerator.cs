...
public static class ProxyGenerator
{
    ...
    private const int TargetFieldAndCtorParameterIndex = 0;
    private const int MethodsFieldAndCtorParameterIndex = 1;
    private const int InvocationFieldAndCtorParameterIndex = 2;
    ...
    private static readonly MethodInfo s_dispatchProxyInvokeMethod = typeof(Invocation).GetMethod("Invoke", BindingFlags.Public | BindingFlags.Instance)!;
    ...
    public static object CreateProxyInstance(IServiceProvider service, ServiceDescriptor descriptor)
    {
        object target = service.CreateInstance(descriptor);
        var chainBuilder = service.GetRequiredService<IInterceptorChainBuilder>();
        Type baseType = target.GetType();
        Type interfaceType = descriptor.ServiceType;
        ...
        return Activator.CreateInstance(proxiedType.GeneratedType, new object[] { target, proxiedType.MethodInfos, new Invocation(service) })!;
    }
    ...
    private sealed class ProxyAssembly
    {
            ...
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type interfaceType)
        {
            ...
            if (!interfaceType.IsInterface)
            {
                ...
                throw new ArgumentException($"The type '{interfaceType.FullName}' must be an interface, not a class.", "T");
            }
            ...
            //    throw new ArgumentException($"The base type '{baseType.FullName}' cannot be sealed.", "TProxy");
            //}

            // The base type cannot be abstract
            //if (baseType.IsAbstract)
            //{
            //    throw new ArgumentException($"The base type '{baseType.FullName}' cannot be abstract.", "TProxy");
            //}

            // The base type must have a public default ctor
            //if (baseType.GetConstructor(Type.EmptyTypes) == null)
            //{
            //    throw new ArgumentException($"The base type '{baseType.FullName}' must have a public parameterless constructor.", "TProxy");
            //}

            // Create a type that derives from 'baseType' provided by caller
            ProxyBuilder pb = CreateProxy($"{baseType.Name}Proxy", baseType);
            ...
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] Type proxyBaseType)
        {
            ...
            TypeBuilder tb = _mb.DefineType(name + "_" + nextId, TypeAttributes.Public);
    ...
    private sealed class ProxyBuilder
    {
            ...
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] Type proxyBaseType)
        {
            _assembly = assembly;
            _tb = tb;
            _proxyBaseType = typeof(object);

            _fields = new List<FieldBuilder>();
            _fields.Add(tb.DefineField("_target", proxyBaseType, FieldAttributes.Private));
            _fields.Add(tb.DefineField("_methods", typeof(MethodInfo[]), FieldAttributes.Private));
            _fields.Add(tb.DefineField("_invocation", typeof(Invocation), FieldAttributes.Private));
        ...
        private MethodBuilder AddMethodImpl(MethodInfo mi, int methodInfoIndex)
        {
            ...
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldfld, _fields[MethodsFieldAndCtorParameterIndex]); // MethodInfo[] _methodInfos
            ...
            // object result = _invocation.Invoke(null, methodInfo, args);
            LocalBuilder? resultLocal = mi.ReturnType != typeof(void) ? il.DeclareLocal(typeof(object)) : null;

            //_invocation
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldfld, _fields[InvocationFieldAndCtorParameterIndex]);

            //Target
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldfld, _fields[TargetFieldAndCtorParameterIndex]);
            //methodInfo
            il.Emit(OpCodes.Ldloc, methodInfoLocal);
            //Args
            argsArr.Load();
