        ...
        public static void Emit(GeneratorExecutionContext context, IReadOnlyList<DbContextClass> dbContextClasses)
        {
            ...
            foreach (var dbContextClass in dbContextClasses)
            {
                foreach (var dbContextProperty in dbContextClass.Properties)
                {
                    ...
                    var output = template.Render(new { GeneratorVersion = generatorVersion, Namespaces = orderdNamespaces, Namespace = $"{dbContextProperty.Namespace.Substring(0, entitiesIndex)}.Repositories", DbContext = dbContextClass.Name, dbContextProperty.PrimaryKey, dbContextProperty.Name }, member => member.Name);
                    context.AddSource($"Skywalker.Ddd.DomainServices.{dbContextProperty.Name}.g.cs", SourceText.From(output, Encoding.UTF8));
                }
            }
