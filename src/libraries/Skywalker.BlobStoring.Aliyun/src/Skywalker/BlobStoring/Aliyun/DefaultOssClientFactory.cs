...
{
    protected ICaching Cache { get; }

    public DefaultOssClientFactory(
        ICachingProvider _cache)
    {
        Cache = _cache.GetCaching("Skywalker.BlobStoring.Aliyun.DefaultOssClientFactory");
    }
    ...
    protected virtual IOss GetSecurityTokenClient(AliyunBlobProviderConfiguration configuration)
    {
        ...
        var cacheItem = Cache.Get<AliyunTemporaryCredentialsCacheItem>(configuration.TemporaryCredentialsCacheKey);
        if (cacheItem == null)
        {
        ...
        AssumeRole_Credentials credentials)
    {
        ...
        Cache.Set(configuration.TemporaryCredentialsCacheKey,
            temporaryCredentialsCache, TimeSpan.FromSeconds(configuration.DurationSeconds - 10));
