...
public abstract class AbstractDynamicLinqCustomTypeProvider
{
    ...
    protected static IEnumerable<Type?> FindTypesMarkedWithDynamicLinqTypeAttribute(IEnumerable<Assembly> assemblies)
    {
    ...
    protected static Type? ResolveTypeBySimpleName(IEnumerable<Assembly> assemblies, string simpleTypeName)
    {
        ...
        foreach (var assembly in assemblies)
        {
            ...
            var firstMatchingFullname = fullnames.FirstOrDefault(fn => fn?.EndsWith($".{simpleTypeName}") == true);
    ...
    /// <param name="assemblies">The assemblies to process.</param>
    /// <returns><see cref="IEnumerable{Type}" /></returns>
    protected static IEnumerable<Type?> GetAssemblyTypesWithDynamicLinqTypeAttribute(IEnumerable<Assembly> assemblies)
    {
        Check.NotNull(assemblies, nameof(assemblies));
