...
public class HtmlNodeOps : IElementOps<HtmlNode>
{
    ...
    public virtual Selector<HtmlNode> Type(NamespacePrefix prefix, string type)
    {
        return prefix.IsSpecific
             ? (nodes => Enumerable.Empty<HtmlNode>())
    ...
    private static Selector<HtmlNode>? GetSelector(ISelectorGenerator subgenerator)
    {
        return ((SelectorGenerator<HtmlNode>)subgenerator).Selector;
    }

    private static IEnumerable<HtmlNode> Split(ISelectorGenerator subgenerator, HtmlNode parent, bool keepBefore, bool keepAfter)
    {
        ...
        if (keepAfter)
        {
            yield return CreateNodesGroup(doc, children, splitterPositions[^1] + (keepSeparators ? 0 : 1), children.Length - 1);
        }
    }
    ...
    private static int? IndexOfChild(ISelectorGenerator subgenerator, HtmlNode parent, int startIndex)
    {
        var selector = GetSelector(subgenerator);

        var children = parent.ChildNodes;
        var limit = selector?.Invoke(new[] { parent })
    ...
    private static HtmlNode CreateNodesGroup(HtmlDocument doc, IList<HtmlNode> nodes, int start, int last)
    {
    ...
    public Selector<HtmlNode> Not(ISelectorGenerator subgenerator)
    {
        ...
        return nodes =>
        {
            var matches = compiled?.Invoke(nodes.Select(x => x.ParentNode)).ToList();
