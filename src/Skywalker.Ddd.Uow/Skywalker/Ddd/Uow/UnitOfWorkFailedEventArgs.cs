...
{
    /// <summary>
    /// Exception that caused failure. This is set only if an error occurred during <see cref="IUnitOfWork.CompleteAsync(CancellationToken)"/>.
    /// Can be null if there is no exception, but <see cref="IUnitOfWork.CompleteAsync(CancellationToken)"/> is not called. 
