...
namespace Skywalker.Queries.Abstractions
{
    ...
    public interface ISearchingPipelineBehavior<in TQuery, TOutput> where TQuery : IQuery<TOutput>
    {
        ...
        Task<TOutput> HandleAsync(TQuery querier, CancellationToken cancellationToken, QueryHandlerDelegate<TOutput> next);
    }
    ...
    public interface ISearchingPipelineBehavior<TOutput>
    {
        ...
        /// <param name="next">Awaitable delegate for the next action in the pipeline. Eventually this delegate represents the handler.</param>
        /// <returns>Awaitable task returning the <typeparamref name="TOutput"/></returns>
        Task<TOutput> HandleAsync(CancellationToken cancellationToken, QueryHandlerDelegate<TOutput> next);
    }
}
