...
public static class ObjectHelper
{
    ...
    {
        var cacheKey = $"{obj?.GetType().FullName}-{propertySelector}-{(ignoreAttributeTypes != null ? "-" + string.Join("-", ignoreAttributeTypes.Select(x => x.FullName)) : "")}";

        var property = s_cachedObjectProperties.GetOrAdd(cacheKey, PropertyFactory);
