...
public static class ReaderWriterLockSlimExtensions
{
    ...
    public static TResult ReadWriteLocking<TResult>(this ReaderWriterLockSlim locker, Func<TResult> shared, Func<TResult> exclusive)
    {
        return locker.ReadWriteLocking<object, TResult>((@object) => shared(), (@object) => exclusive(), default);
    }

    public static TResult ReadWriteLocking<T, TResult>(this ReaderWriterLockSlim locker, Func<T?, TResult> shared, Func<T?, TResult> exclusive, T? @object)
    {
        try
        {
            ...
            try
            {
                if (!locker.TryEnterWriteLock(3000))
                {
    ...
    public static async Task<TResult> ReadWriteLockingAsync<TResult>(this ReaderWriterLockSlim locker, Func<Task<TResult>> shared, Func<Task<TResult>> exclusive)
    {
        return await locker.ReadWriteLockingAsync<object, TResult>(async (@object) => await shared(), async (@object) => await exclusive(), default);
    }

    public static async Task<TResult> ReadWriteLockingAsync<T, TResult>(this ReaderWriterLockSlim locker, Func<T?, Task<TResult>> shared, Func<T?, Task<TResult>> exclusive, T? @object)
    {
        try
        {
            locker.EnterUpgradeableReadLock();
            var result = await shared(@object);
            if (result != null)
            {
                return result;
            }
