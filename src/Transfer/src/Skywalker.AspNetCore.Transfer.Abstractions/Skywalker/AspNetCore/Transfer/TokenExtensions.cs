...
{
    /// <summary>
    /// Extension methods for storing authentication tokens in <see cref="TransferProperties"/>.
    /// </summary>
    public static class AuthenticationTokenExtensions
    {
        ...
        public static void StoreTokens(this TransferProperties properties, IEnumerable<TransferToken> tokens)
        {
            if (properties == null)
            {
        ...
        public static string? GetTokenValue(this TransferProperties properties, string tokenName)
        {
            if (properties == null)
            {
                throw new ArgumentNullException(nameof(properties));
        ...
        /// </summary>
        /// <param name="properties">The <see cref="TransferProperties"/> to update.</param>
        /// <param name="tokenName">The token name.</param>
        /// <param name="tokenValue">The token value.</param>
        /// <returns><see langword="true"/> if the token was updated, otherwise <see langword="false"/>.</returns>
        public static bool UpdateTokenValue(this TransferProperties properties, string tokenName, string tokenValue)
        {
            if (properties == null)
            {
                throw new ArgumentNullException(nameof(properties));
        ...
        /// <summary>
        /// Returns all of the <see cref="TransferToken"/> instances contained in the properties.
        /// </summary>
        /// <param name="properties">The <see cref="TransferProperties"/> properties.</param>
        /// <returns>The authentication tokens.</returns>
        public static IEnumerable<TransferToken> GetTokens(this TransferProperties properties)
        {
            if (properties == null)
            {
                throw new ArgumentNullException(nameof(properties));
            ...
            var tokens = new List<TransferToken>();
            if (properties.Items.TryGetValue(TokenNamesKey, out var value) && !string.IsNullOrEmpty(value))
            {
                ...
                foreach (var name in tokenNames)
                {
                    ...
                    if (token != null)
                    {
                        tokens.Add(new TransferToken { Name = name, Value = token });
                    }
                }
            }
        ...
        /// </summary>
        /// <param name="auth">The <see cref="ITransferService"/>.</param>
        /// <param name="context">The <see cref="HttpContext"/> context.</param>
        /// <param name="tokenName">The name of the token.</param>
        /// <returns>The value of the token if present.</returns>
        public static Task<string?> GetTokenAsync(this ITransferService auth, HttpContext context, string tokenName)
        ...
        /// <param name="tokenName">The name of the token.</param>
        /// <returns>The value of the token if present.</returns>
        public static async Task<string?> GetTokenAsync(this ITransferService auth, HttpContext context, string? scheme, string tokenName)
        {
            if (auth == null)
            {
            ...
            var result = await auth.TransferAsync(context, scheme);
