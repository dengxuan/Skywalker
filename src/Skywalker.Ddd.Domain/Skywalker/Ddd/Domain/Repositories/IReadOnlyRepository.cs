...
public interface IReadOnlyRepository<TEntity> : IRepository, IQueryable<TEntity> where TEntity : class, IEntity
{
    ...
    /// <summary>
    /// Gets a paged list of the entities by the given <paramref name="sorting"/> and <paramref name="filter"/>
    /// </summary>
    /// <param name="skip">To skiping count</param>
    /// <param name="limit">A limit of the page size</param>
    /// <param name="sorting">The sorting rules for this pagination</param>
    /// <param name="filter">The filter rules for this pagination</param>
    /// <param name="cancellationToken">A <see cref="CancellationToken" /> to observe while waiting for the task to complete.</param>
    /// <returns>Entities with no more than <paramref name="limit"/> after filtering by <paramref name="filter"/> and skipping a <paramref name="skip"/> of rows</returns>
    ...
    /// <summary>
    /// Get a single entity by the given <paramref name="specification"/>.
    /// It returns null if no entity matches the specification.
    /// It throws <see cref="InvalidOperationException"/> if there are multiple entities matching the specification.
    /// </summary>
    /// <param name="specification">A specification to find the entity</param>
    /// <param name="cancellationToken">A <see cref="CancellationToken" /> to observe while waiting for the task to complete.</param>
    Task<TEntity?> FindAsync(ISpecification<TEntity> specification, CancellationToken cancellationToken = default);

    /// <summary>
    /// Get a single entity by the given <paramref name="specification"/>.
    /// It throws <see cref="EntityNotFoundException"/> if there is no entity matching the specification.
    /// It throws <see cref="InvalidOperationException"/> if there are multiple entities matching the specification.
    /// </summary>
    ...
    /// <summary>
    /// Gets a list of entities matching the given <paramref name="specification"/>.
    /// </summary>
    /// <param name="specification">A specification to filter entities</param>
    /// <param name="cancellationToken">A <see cref="CancellationToken" /> to observe while waiting for the task to complete.</param>
    ...
    /// <summary>
    /// Gets a paged list of entities matching the given <paramref name="specification"/>.
    /// </summary>
    /// <param name="specification">A specification to filter entities</param>
    ...
    /// <param name="sorting">Sorting rules for this pagination</param>
    /// <param name="cancellationToken">A <see cref="CancellationToken" /> to observe while waiting for the task to complete.</param>
    /// <returns>Paged list of entities matching the specification</returns>
