...
public interface IBasicRepository<TEntity> : IReadOnlyRepository<TEntity> where TEntity : class, IEntity
{
    ...
    Task<int> InsertAsync(IEnumerable<TEntity> entities, bool autoSave = false, CancellationToken cancellationToken = default);
    ...
    /// <param name="autoSave">Set true to automatically save changes to database. Default: false</param>
    /// <param name="cancellationToken">A <see cref="CancellationToken" /> to observe while waiting for the task to complete.</param>
    Task DeleteAsync(Expression<Func<TEntity, bool>> filter, bool autoSave = false, CancellationToken cancellationToken = default);
}
